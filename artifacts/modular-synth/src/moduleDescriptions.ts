export const MODULE_DESCRIPTIONS: Record<string, string> = {
  ...
  sampler:      '8-bank sample player for your own audio files. Click a bank dot to select a slot, then LOAD to decode a file into it — WAV, MP3, FLAC, and OGG are all accepted. GATE triggers playback and tracks keyboard pitch when patched from a keyboard gate output; the PITCH knob offsets by ±24 semitones on top. ATK fades the sample in over 0–2 s from silence to full level on each trigger — useful for softer attacks or swell-in effects. START and LEN trim the playback region and both take effect immediately while the sample is playing. BANK switches slots; FWD/REV flips direction; LOOP sustains until gate-off. EOC fires a short pulse the moment the sample finishes — patch it to a sequencer reset or another GATE for chained playback.',
  ...
  poly_step:      '8-track polyrhythmic step sequencer. Each track has independent step length (4/8/12/16), per-step accent (right-click), probability, mute, and velocity. Gate outputs: one per track (KICK–AUX). Global outputs: clock passthrough, beat pulse (step 0), master EOC, position CV, and step CV. Velocity CV per track scales with accent. Inputs: external clock, reset, run/stop toggle, BPM CV, and swing CV. Click a track label to randomize its pattern; right-click to clear it.',
};
