...
## MIDI Start (0xFA) → phase-align all clocks to beat 1

Feature: when the DAW sends MIDI Start and the clock is LOCKED, all internal clock timers should snap to beat 0 so the synth's drum machine, clock_gen, clock_mul, etc. all start together at bar 1.

**Rule:** Keep a Set of restarter functions (`_activeClockRestarters`) in audioEngine.ts. `makeClockTimer` registers its restart closure when created and removes it in `destroy`. `restartAllClockTimers()` (exported) iterates and fires them all simultaneously.  In `useMIDI`, 0xFA fires `onStartRef.current()` immediately (0xFA arrives at or just before the downbeat). SynthApp's `handleMidiStart` callback checks `midiClockInfoRef.current.locked` and calls `restartAllClockTimers()` if true. 0xFB (Continue) and 0xFC (Stop) do NOT restart — Continue resumes from current position; Stop leaves the grid as-is.

**Why:** Parallel restart via the Set means all timers in the worker receive their `restart` messages in the same JS task, so they all snap to `now + intervalMs` at essentially the same moment. No timer gets a head start. Workers free-run from there, so this is a one-shot phase correction (not a continuous PLL), but it correctly handles the "start from bar 1" use case which is by far the most common sync operation.
